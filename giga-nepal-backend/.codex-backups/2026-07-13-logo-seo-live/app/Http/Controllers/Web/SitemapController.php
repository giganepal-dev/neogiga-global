...
class SitemapController extends Controller
{
    ...
    public function __invoke(): Response
    {
        ...
        $cacheKey = 'seo:sitemap-index:' . request()->getHost() . ':' . sha1(json_encode($state));
        ...
        $xml = Cache::remember($cacheKey, 3600, function () use ($marketplace, $state) {
            ...
            foreach (['categories', 'products'] as $section) {
    ...
    {
        abort_unless(in_array($section, ['pages', 'categories', 'products'], true) && $page > 0, 404);
        ...
        $cacheKey = 'seo:sitemap-section:' . request()->getHost() . ":{$section}:{$page}:" . sha1($fingerprint);
    ...
    /** @return array{categories_count:int,categories_lastmod:?string,products_count:int,products_lastmod:?string} */
