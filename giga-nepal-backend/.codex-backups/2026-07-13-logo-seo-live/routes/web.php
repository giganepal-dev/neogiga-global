...
Route::get('/manufacturers/{slug}', fn (\Illuminate\Http\Request $request, string $slug) => redirect()->to('/manufacturer/' . $slug . ($request->getQueryString() ? '?' . $request->getQueryString() : ''), 301))->where('slug', '[a-z0-9\-]+');
Route::get('/brand/{slug}', [SeoLandingController::class, 'brand'])->where('slug', '[a-z0-9\-]+')->name('brand.show');
Route::get('/brands/{slug}', fn (\Illuminate\Http\Request $request, string $slug) => redirect()->to('/brand/' . $slug . ($request->getQueryString() ? '?' . $request->getQueryString() : ''), 301))->where('slug', '[a-z0-9\-]+');
...
Route::get('/sitemaps/{section}-{page}.xml', [SitemapController::class, 'section'])
    ->whereIn('section', ['pages', 'categories', 'products'])
    ->whereNumber('page');
