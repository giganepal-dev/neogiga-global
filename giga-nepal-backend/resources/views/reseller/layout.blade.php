...
<head>
    ...
    <title>@yield('title', 'Reseller Portal') — NeoGiga</title>
    <style>
...
<div class="top-bar"><div class="wrap">
    <a href="/reseller" class="brand" style="text-decoration:none">NeoGiga Reseller</a>
    <form method="post" action="/reseller/logout" style="margin:0">@csrf<button class="btn btn-ghost" type="submit">Sign Out</button></form>
</div></div>
<div class="wrap">
<div class="main-grid">
    <nav class="sidebar">
        <a href="/reseller" class="{{ request()->is('reseller') ? 'active' : '' }}">Dashboard</a>
        <a href="/reseller/profile" class="{{ request()->is('reseller/profile') ? 'active' : '' }}">Profile</a>
        <a href="/reseller/products" class="{{ request()->is('reseller/products') ? 'active' : '' }}">Products</a>
        <a href="/reseller/orders" class="{{ request()->is('reseller/orders') ? 'active' : '' }}">Orders</a>
    </nav>
