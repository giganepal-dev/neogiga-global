...
<head>
    ...
    <title>@yield('title', 'Business Portal') — NeoGiga</title>
    <style>
...
<div class="top-bar"><div class="wrap">
    <a href="/b2b" class="brand" style="text-decoration:none">NeoGiga Business</a>
    <form method="post" action="/b2b/logout" style="margin:0">@csrf<button class="btn btn-ghost" type="submit">Sign Out</button></form>
</div></div>
<div class="wrap">
<div class="main-grid">
    <nav class="sidebar">
        <a href="/b2b" class="{{ request()->is('b2b') ? 'active' : '' }}">Dashboard</a>
        <a href="/b2b/products" class="{{ request()->is('b2b/products') ? 'active' : '' }}">Products</a>
        <a href="/b2b/rfqs" class="{{ request()->is('b2b/rfqs') ? 'active' : '' }}">RFQs</a>
        <a href="/b2b/orders" class="{{ request()->is('b2b/orders') ? 'active' : '' }}">Orders</a>
    </nav>
