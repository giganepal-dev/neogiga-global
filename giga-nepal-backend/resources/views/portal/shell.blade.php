...
<head>
    ...
    <title>@yield('title', $portal['name']) · NeoGiga {{ $portal['name'] }}</title>
    ...
    <x-icon-styles/>
    <style>
        :root{--navy:#0F172A;--line:rgba(148,163,184,.16);--muted:#64748B;--accent:#f9bd2c;--ok:#16a34a;--warn:#d97706;--bad:#e5484d;--info:#0f62e6;--ng-accent:#f9bd2c;--ng-focus:#f9bd2c}
        *{box-sizing:border-box}body{margin:0;font:15px/1.5 ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,Arial;color:#0f172a;background:#f4f6fa}
        ...
        .kpis,.kpi-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:18px}
        ...
        .tbl th,.tbl td,.table th,.table td{text-align:left;padding:9px 14px;border-bottom:1px solid var(--line)}
        .tbl th,.table th{font-size:.72rem;text-transform:uppercase;letter-spacing:.05em;color:var(--muted)}
...
<div class="shell">
    ...
    <main class="main">
        <div class="topbar">
            ...
            <span class="badge b-muted">{{ $vendor->name ?? $distributor->name ?? $reseller->name ?? $manufacturer->name ?? $account->company_name ?? $account->name ?? '' }}</span>
        </div>
