        ...
        <div class="grid product-primary-grid" style="grid-template-columns:minmax(300px,.9fr) minmax(0,1.4fr) 340px;align-items:start">
            ...
            <section class="panel" style="padding:22px">
                ...
                <div style="display:flex;gap:10px;flex-wrap:wrap;margin:12px 0 18px">
                    ...
                    @endif
                    @if($product->mpn)
                        @if($product->mpn)<a class="badge b-muted" href="/mpn/{{ str_replace('/','--', urlencode($product->mpn)) }}">MPN: {{ $product->mpn }}</a>@endif
                    @endif
                </div>
...
<section class="section product-detail-section">
    <div class="wrap grid" style="grid-template-columns:repeat(auto-fit,minmax(280px,1fr))">
        ...
        <div class="info-card"><h2><x-icon name="quality" size="18"/> Certifications &amp; Compliance</h2><x-product-certification-marks :certifications="$certifications" :compact="false" /><p class="sub" style="margin-bottom:0">Marks appear only when NeoGiga has a verified, current product record. Confirm market-specific requirements before purchase.</p></div>
