...
<section class="account-panel">
    ...
    @if($applications->isEmpty())<div class="account-empty">No partner applications submitted.</div>@else<div class="account-table-wrap"><table class="account-table"><thead><tr><th>Application</th><th>Role</th><th>Company</th><th>Status</th><th>Submitted</th></tr></thead><tbody>@foreach($applications as $application)<tr><td class="mono">{{ $application->application_number }}</td><td>{{ ucwords(str_replace('_',' ',$application->role_key)) }}</td><td>{{ $application->company_name }}</td><td><span class="account-badge {{ $application->status }}">{{ str_replace('_',' ',$application->status) }}</span></td><td>{{ $application->submitted_at ? \Carbon\Carbon::parse($application->submitted_at)->format('d M Y') : 'Draft' }}</td></tr>@endforeach</tbody></table></div>@endif
</section>
<section class="account-panel">
