    ...
    @if($products->count())
        <div class="grid" style="grid-template-columns:repeat(auto-fill,minmax(230px,1fr))">
            @foreach($products as $product)
                ...
                <article class="product-card"><a href="{{ $publicBase }}/products/{{ $product->slug }}"><div class="product-img"><x-product-image-badges :product="$product" /><img src="{{ $image?->publicUrl() ?: url('/images/products/neogiga-product-placeholder-2026.png') }}" alt="{{ $image?->alt_text ?: $product->name }}" width="480" height="360" loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:contain;background:transparent"></div><h3>{{ $product->name }}</h3></a><p class="sub">@if($product->mpn)<a href="/mpn/{{ str_replace('/','--', urlencode($product->mpn)) }}">{{ $product->mpn }}</a>@else<a href="{{ $publicBase }}/products?q={{ urlencode($product->sku) }}">{{ $product->sku }}</a>@endif @if($product->category) · <a href="{{ $publicBase }}/categories/{{ $product->category->slug }}">{{ $product->category->name }}</a>@endif</p><a class="btn btn-ghost" href="{{ $publicBase }}/products/{{ $product->slug }}">View product</a></article>
            @endforeach
        </div>
