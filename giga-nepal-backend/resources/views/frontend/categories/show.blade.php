    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:14px">
        @foreach ($products as $p)
            ...
            <article style="background:rgba(13,34,64,.55);border:1px solid var(--line);border-radius:var(--r);padding:16px">
                ...
                <div style="color:var(--muted);font-size:.82rem" class="mono">@if($p->sku)<a href="{{ $publicBase }}/products?q={{ urlencode($p->sku) }}">{{ $p->sku }}</a>@endif @if($p->mpn)· <a href="/mpn/{{ urlencode($p->mpn) }}">{{ $p->mpn }}</a>@endif</div>
            </article>
        @endforeach
    </div>
