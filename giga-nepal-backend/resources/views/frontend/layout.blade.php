...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}html{scroll-behavior:smooth}
        ...
        .category-hero{min-width:0}.category-hero>div{min-width:0}
        ...
        @media(max-width:980px){.head-main{grid-template-columns:1fr;gap:10px;padding:12px 0}.search-tools,.head-actions{overflow-x:auto}.search-tools{padding-bottom:2px}.search{min-width:320px}.header-tool span,.icon-btn .action-label{display:inline}.nav-row{display:none}.hero-grid,.layout-2,.product-primary-grid,.product-review-grid{grid-template-columns:1fr!important}.product-primary-grid .product-gallery-main{min-height:0}.foot-grid{grid-template-columns:1fr 1fr}.footer-editions{grid-template-columns:1fr}.filter{position:static}.mobile-bottom{display:flex;position:fixed;left:0;right:0;bottom:0;z-index:55;background:rgba(11,15,17,.95);backdrop-filter:blur(12px);border-top:1px solid var(--line);justify-content:space-around;padding:8px 6px}.mobile-bottom a{color:var(--muted);font-size:.75rem;font-weight:600;text-align:center}.float-ai{bottom:64px}.hero h1{font-size:3rem}}
        @media(max-width:620px){.wrap{width:min(var(--max),calc(100% - 24px))}.hero-grid{padding:44px 0}.hero h1{font-size:2.5rem}.section{padding:44px 0}.foot-grid{grid-template-columns:1fr 1fr}.ai-bar{display:grid}.category-grid{grid-template-columns:1fr 1fr}.section-head{display:block}.btn{width:100%}.top-strip .wrap{justify-content:center}.footer-edition-links button{max-width:100%}.category-hero{align-items:flex-start!important}.category-hero img{width:72px!important;height:72px!important}}
