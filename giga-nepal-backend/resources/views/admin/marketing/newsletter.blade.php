...
<div class="grid" style="grid-template-columns:1fr 1fr;align-items:start;margin-bottom:16px">
    <div class="card"><div class="card-h"><h2>Create Newsletter Template</h2></div><form method="post" action="/admin/marketing/newsletter/templates" style="padding:16px;display:grid;gap:10px">@csrf
        ...
        <input name="subject" maxlength="190" placeholder="Subject" style="height:38px;border:1px solid var(--line);border-radius:7px;padding:0 10px">
        <textarea name="text_body" rows="4" placeholder="Plain text body" style="border:1px solid var(--line);border-radius:7px;padding:10px"></textarea>
        <button class="btn btn-primary" type="submit">Create Template</button>
    </form></div>
    <div class="card"><div class="card-h"><h2>Create Newsletter Campaign</h2></div><form method="post" action="/admin/marketing/newsletter/campaigns" style="padding:16px;display:grid;gap:10px">@csrf
...
<div class="card" style="margin-top:16px"><div class="card-h"><h2>Newsletter Categories</h2></div><div style="padding:16px;display:flex;gap:8px;flex-wrap:wrap">@foreach($categories as $c)<span class="badge b-info">{{ $c->name }}</span>@endforeach</div></div>
@endsection
