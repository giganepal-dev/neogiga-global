...
<section class="card" style="margin-bottom:16px">
    ...
    <div class="scroll-x"><table class="tbl"><thead><tr><th>Supplier</th><th>Policy</th><th>Import</th><th>Media</th><th>Rate Limit</th><th>Last Sync</th><th>Actions</th></tr></thead><tbody>
        @forelse($sources as $source)
            ...
            <tr>
                <td><strong>{{ $source->name }}</strong><div class="sub mono">{{ $source->code }}</div><div class="sub">{{ str_replace('_', ' ', $source->source_type ?? 'supplier') }}</div></td>
                ...
                <td class="actions">
                    @if(($source->source_type ?? 'supplier') !== 'supplier_document')
                    ...
                    @else
                    <span class="badge b-muted">document staging</span>
                    @endif
                </td>
            </tr>
...
<section class="card" style="margin-bottom:16px">
    ...
    <div class="scroll-x"><table class="tbl"><thead><tr><th>Product</th><th>Supplier</th><th>Task</th><th>Quality</th><th>Status</th><th>Action</th></tr></thead><tbody>
        @forelse($tasks as $task)
            ...
            <tr><td><strong>{{ $task->product_name ?: $task->supplier_product_name ?: 'Unmatched source record' }}</strong><div class="sub mono">{{ $task->product_sku ?: $task->supplier_sku ?: 'No SKU' }}</div></td><td>{{ $task->source_name ?: $task->source_code ?: '—' }}</td><td><span class="badge b-muted">{{ str_replace('_',' ',$task->task_type) }}</span><div class="sub">{{ implode(', ', array_slice((array) ($evidence['missing_fields'] ?? []), 0, 3)) ?: 'No missing-field summary' }}</div></td><td><span class="badge {{ $quality >= 80 ? 'b-ok' : ($quality >= 60 ? 'b-warn' : 'b-danger') }}">{{ number_format($quality, 0) }}/100</span></td><td><span class="badge {{ $task->status === 'resolved' ? 'b-ok' : ($task->status === 'deferred' ? 'b-warn' : 'b-muted') }}">{{ $task->status }}</span></td><td class="actions">@if($task->supplier_product_id && $task->product_id)<details class="modal"><summary class="btn btn-ghost">Identity</summary><div class="modal-panel"><div class="modal-h"><h3>Verify Product Identity</h3><span class="badge b-warn">no auto merge</span></div><form class="modal-b form-stack" method="post" action="/admin/catalog-ingestion/review-tasks/{{ $task->id }}/identity">@csrf<div class="field"><label>Manufacturer / brand</label><input class="control" name="manufacturer" value="{{ $task->source_manufacturer ?: $task->source_brand }}" required></div><div class="field"><label>Manufacturer part number</label><input class="control mono" name="mpn" value="{{ $task->manufacturer_part_number }}" required></div><div class="field"><label>Verification note</label><textarea class="control" name="note" required placeholder="Evidence used to verify the manufacturer and MPN"></textarea></div><div class="note">This only records identity and creates a follow-up review. It does not merge products, publish content, set stock, or set a marketplace price.</div><button class="btn btn-primary" type="submit">Record Identity</button></form></div></details>@endif <details class="modal"><summary class="btn btn-ghost">Review</summary><div class="modal-panel"><div class="modal-h"><h3>Review Task #{{ $task->id }}</h3></div><form class="modal-b form-stack" method="post" action="/admin/catalog-ingestion/review-tasks/{{ $task->id }}">@csrf<div class="field"><label>Status</label><select class="control" name="status">@foreach(['open','deferred','resolved'] as $status)<option value="{{ $status }}" @selected($task->status === $status)>{{ $status }}</option>@endforeach</select></div><div class="field"><label>Review note</label><textarea class="control" name="note" required>{{ $evidence['review_note'] ?? '' }}</textarea></div><button class="btn btn-primary" type="submit">Save Review</button></form></div></details></td></tr>
