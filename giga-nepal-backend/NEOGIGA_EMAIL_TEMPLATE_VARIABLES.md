...
## Supported Variables
`{{customer_name}}`, `{{email}}`, `{{phone}}`, `{{country}}`, `{{region}}`, `{{marketplace_name}}`, `{{order_number}}`, `{{order_total}}`, `{{currency}}`, `{{cart_items}}`, `{{product_name}}`, `{{category_name}}`, `{{tracking_number}}`, `{{invoice_url}}`, `{{unsubscribe_url}}`, `{{otp_code}}`, `{{expiry_minutes}}`.
