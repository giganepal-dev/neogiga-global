...
if (config('neogiga_global.features.locale_prefix_routes', true)) {
    ...
    Route::prefix('{localePrefix}')
        ...
        ->group(function () {
            ...
            Route::get('/ai', fn (string $localePrefix) => app($aiRobotics)->index())->name('localized.ai-robotics');
            Route::get('/ai/robots', fn (string $localePrefix) => app($aiRobotics)->robots(app(Request::class)))->name('localized.ai-robotics.robots');
            Route::get('/ai/robots/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->robotDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.robot');
            Route::get('/ai/ai-models', fn (string $localePrefix) => app($aiRobotics)->aiModels(app(Request::class)))->name('localized.ai-robotics.ai-models');
            Route::get('/ai/ai-models/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->aiModelDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.ai-model');
            Route::get('/ai/store', fn (string $localePrefix) => app($aiRobotics)->store(app(Request::class)))->name('localized.ai-robotics.store');
            Route::get('/ai/learning', fn (string $localePrefix) => app($aiRobotics)->learning())->name('localized.ai-robotics.learning');
            Route::get('/ai/learning/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->learningPathDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.learning-path');
            Route::get('/ai/lab', fn (string $localePrefix) => app($aiRobotics)->lab())->name('localized.ai-robotics.lab');
            Route::get('/ai/institutional', fn (string $localePrefix) => app($aiRobotics)->institutional())->name('localized.ai-robotics.institutional');
            Route::get('/ai/manufacturers', fn (string $localePrefix) => app($aiRobotics)->manufacturers())->name('localized.ai-robotics.manufacturers');
            Route::get('/ai/manufacturers/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->manufacturerDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.manufacturer');
            Route::get('/ai/events', fn (string $localePrefix) => app($aiRobotics)->events())->name('localized.ai-robotics.events');
            Route::get('/ai/news', fn (string $localePrefix) => app($aiRobotics)->news(app(Request::class)))->name('localized.ai-robotics.news');
            Route::get('/ai/news/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->articleDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.article');
            Route::get('/ai/integrators', fn (string $localePrefix) => app($aiRobotics)->integrators())->name('localized.ai-robotics.integrators');
            Route::get('/ai/integrators/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->integratorDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.integrator');
            Route::get('/ai/compare', fn (string $localePrefix) => app($aiRobotics)->compare(app(Request::class)))->name('localized.ai-robotics.compare');
            Route::get('/ai/institutional/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->institutionalDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.institutional-detail');
            Route::get('/ai/demo-request', fn (string $localePrefix) => app($aiRobotics)->demoRequestForm())->name('localized.ai-robotics.demo-request');
            Route::post('/ai/demo-request', fn (string $localePrefix, Request $request) => app($aiRobotics)->storeDemoRequest($request))->name('localized.ai-robotics.demo-request.store');
            Route::get('/ai/lab/book', fn (string $localePrefix) => app($aiRobotics)->labBookingForm())->name('localized.ai-robotics.lab-booking');
            Route::post('/ai/lab/book', fn (string $localePrefix, Request $request) => app($aiRobotics)->storeLabBooking($request))->name('localized.ai-robotics.lab-booking.store');
            Route::get('/ai/events/{slug}', fn (string $localePrefix, string $slug) => app($aiRobotics)->eventDetail($slug))->where('slug', '[a-z0-9\-]+')->name('localized.ai-robotics.event');
            Route::post('/ai/events/{slug}/register', fn (string $localePrefix, Request $request, string $slug) => app($aiRobotics)->storeEventRegistration($request, $slug))->name('localized.ai-robotics.event.register');
