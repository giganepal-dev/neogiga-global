...
if (config('pcb.enabled', true)) {
    Route::domain(config('pcb.domain', 'pcb.neogiga.com'))->group(function () {
        ...
        Route::middleware('pcb.auth')->group(function () {
            ...
            Route::post('/en/projects/{project}/quotes/{quote}/approve', [PcbPortalController::class, 'approveQuote'])->middleware('throttle:6,1')->name('pcb.quotes.approve');
            Route::post('/en/projects/{project}/quotes/{quote}/reject', [PcbPortalController::class, 'rejectQuote'])->middleware('throttle:6,1')->name('pcb.quotes.reject');
            Route::post('/en/projects/{project}/cancel', [PcbPortalController::class, 'cancel'])->middleware('throttle:6,1')->name('pcb.projects.cancel');
        });
    });
}
...
Route::prefix('admin')->group(function () {
    ...
    Route::middleware('admin.web')->group(function () {
        ...
        Route::post('catalog-ingestion/review-tasks/{task}/identity', [AdminCatalogIngestion::class, 'verifyIdentity'])->whereNumber('task')->middleware('throttle:20,1');
        Route::post('catalog-ingestion/category-mappings/{mapping}', [AdminCatalogIngestion::class, 'reviewCategoryMapping'])->whereNumber('mapping')->middleware('throttle:20,1');
...
if (config('neogiga_global.features.locale_prefix_routes', true)) {
    ...
    Route::prefix('{localePrefix}')
        ...
        ->group(function () {
            ...
            Route::get('/products/{slug}', fn (string $localePrefix, string $slug, \Illuminate\Http\Request $request) => app(\App\Http\Controllers\Web\ProductPageController::class)->show($request, $slug))->where('slug', '[a-z0-9\-]+')->name('localized.products.show');
