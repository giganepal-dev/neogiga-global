...
class ResellerPortalController extends Controller
{
    ...
    { if (Auth::check() && $c->resellerFor(Auth::user())) return redirect('/reseller'); return view('reseller.login'); }
    public function login(Request $r, ResellerContextService $c): RedirectResponse
    {
        ...
        if (!$c->resellerFor(Auth::user())) { Auth::logout(); return back()->withErrors(['email'=>'No reseller account linked.']); }
        return redirect()->intended('/reseller');
    }
    public function logout(Request $r): RedirectResponse { Auth::logout(); $r->session()->invalidate(); $r->session()->regenerateToken(); return redirect('/reseller/login'); }
    ...
    public function profile(Request $r): View { return view('reseller.profile',['r2'=>$r->attributes->get('reseller')]); }
    ...
    public function products(Request $r): View { $r2=$r->attributes->get('reseller'); $p=DB::table('products')->leftJoin('product_categories as c','c.id','=','products.category_id')->select('products.*','c.name as category_name')->where('reseller_id',$r2->id)->orderByDesc('id')->paginate(20); return view('reseller.products',compact('r2','p')); }
    public function orders(Request $r): View { $r2=$r->attributes->get('reseller'); $o=DB::table('orders')->where('reseller_id',$r2->id)->orderByDesc('created_at')->paginate(20); return view('reseller.orders',compact('r2','o')); }
}
