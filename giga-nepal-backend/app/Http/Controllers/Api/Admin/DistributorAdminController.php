...
class DistributorAdminController extends Controller
{
    ...
    public function __construct(private readonly DistributorApprovalService $approval) {}

    public function index(Request $request): JsonResponse
    {
        if (! Schema::hasTable('distributors')) return $this->error('Distributor foundation migration is pending.', 503);
    ...
    {
        if (! Schema::hasTable('distributors')) return $this->error('Distributor foundation migration is pending.', 503);
        if (! Schema::hasTable('distributor_territories')) return $this->error('Distributor territory table is pending migration.', 503);
        $distributor = Distributor::findOrFail($distributor);
    ...
    {
        if (! Schema::hasTable('distributor_commissions')) return $this->error('Distributor commission table is pending migration.', 503);
    ...
    {
        if (! Schema::hasTable('distributor_payouts')) return $this->error('Distributor payout table is pending migration.', 503);
