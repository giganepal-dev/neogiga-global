...
class SendMarketingEmailBatchJob implements ShouldQueue
{
    ...
    public function handle(MarketingEmailProviderManager $providers, RegionalEmailBrandingService $branding, ?EmailProviderConfigurationService $configuration = null): void
    {
        ...
        if (! $sender['verified'] || ! $sender['enabled']) {
            throw new RuntimeException('Marketing sender profile is not verified and enabled.');
        }
        ...
        $messages = DB::table('email_messages')->where('email_campaign_id', $this->campaignId)->where('message_type', 'marketing')->where('status', 'queued')->orderBy('id')->limit($limit)->get();
        if ($messages->isEmpty()) {
            return;
        }

        try {
            $payload = $messages->map(fn ($message) => ['client_reference' => (string) $message->id, 'to' => $message->to_email, 'subject' => $message->subject, 'html' => $message->html_body, 'text' => $message->text_body, 'from' => ['name' => $sender['from_name'], 'email' => $sender['from_email']], 'reply_to' => $sender['reply_to']])->all();
            ...
            DB::transaction(function () use ($messages, $result, $providerName): void {
                ...
                DB::table('email_campaigns')->where('id', $this->campaignId)->update(['status' => 'sending', 'send_cursor' => (int) $messages->max('id'), 'updated_at' => now()]);
                DB::table('email_delivery_circuit_breakers')->updateOrInsert(['provider' => $providerName, 'channel' => 'marketing'], ['state' => 'closed', 'consecutive_failures' => 0, 'opened_at' => null, 'retry_after' => null, 'updated_at' => now(), 'created_at' => now()]);
            });
            ...
            DB::table('email_delivery_circuit_breakers')->updateOrInsert(['provider' => $providerName, 'channel' => 'marketing'], ['state' => $failures >= 3 ? 'open' : 'closed', 'consecutive_failures' => $failures, 'opened_at' => $failures >= 3 ? now() : null, 'retry_after' => $failures >= 3 ? now()->addMinutes(30) : now()->addMinutes(5), 'last_failure_code' => 'provider_batch_failure', 'updated_at' => now(), 'created_at' => now()]);
            throw $exception;
        }

        if (DB::table('email_messages')->where('email_campaign_id', $this->campaignId)->where('message_type', 'marketing')->where('status', 'queued')->exists()) {
            self::dispatch($this->campaignId)->delay(now()->addMinute());
        } else {
            DB::table('email_campaigns')->where('id', $this->campaignId)->where('status', 'sending')->update(['status' => 'completed', 'sent_at' => now(), 'updated_at' => now()]);
        }
    }
}
