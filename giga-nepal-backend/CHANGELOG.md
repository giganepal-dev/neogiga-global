...
- Kept public seller and distributor applications on CSRF-protected same-origin submission routes so storefront proxy redirects cannot convert browser POST requests into GET requests.
- Separated geolocated seller/distributor base country from requested target marketplace countries, persisted the targets on applications, carried them into marketplace approvals or distributor territories during conversion, and displayed base country plus approved/pending sales coverage in both dashboards.
- Replaced the expanded native target-market list with a responsive country dropdown, corrected partner-form dark-mode contrast and page width, and added a required annual-turnover range to seller and distributor applications.
- Restored CSP-hardened styling and interactions by adding the per-request nonce to every trusted inline Blade style/script across storefront, partner portals, PCB, POS and admin views; added source and rendered-response regression checks so strict CSP cannot silently unstyle pages again.
- Restored the storefront's legacy style attributes with a narrowly scoped CSP Level 3 `style-src-attr` compatibility rule while keeping style elements nonce-protected and scripts strict; upgraded product discovery to 48-item progressive pages, compact advanced filters, matching category/brand cards, responsive product detail columns, transparent placeholder treatment and a minimal navigation loading indicator.
- Corrected dark-mode catalog contrast by replacing fixed near-black card/link/action colours with shared theme tokens, retaining accessible dark text on light brand pills, strengthening stock-state contrast and showing the catalog total instead of a misleading zero indexed count.
